"""
An :class:`~msl.qt.widgets.led.LED` widget.
"""
...
class Shapes(IntEnum):
    """Shapes that are available to draw the :class:`.LED`."""
    Circle = 0
    Rounded = 1
    Square = 2
    Triangle = 3
...
class LED(QtWidgets.QWidget):
    ...
    def __init__(self, *, parent=None, shape=Circle, on_color='#0F6900', off_color='grey',
        ...
        *This class is based off of the* `QLed <https://github.com/jazzycamel/QLed>`_ *project*.
    ...
    def toggle(self):
        """Toggle the on/off state of the :class:`LED`."""
        self._is_on = not self._is_on
        self.update()
        self.toggled.emit(self._is_on)

    def turn_off(self):
        """Turn the :class:`LED` off."""
        if self._is_on:
            self.toggle()

    def turn_on(self):
        """Turn the :class:`LED` on."""
        if not self._is_on:
            self.toggle()
    ...
    def mouseReleaseEvent(self, event):
        """Overrides :meth:`QtWidgets.QWidget.mouseReleaseEvent`."""
        if self._is_clickable:
            self.toggle()
        self.clicked.emit()
        super(LED, self).mouseReleaseEvent(event)
